...
// Wo adds one FP32 accumulation to the validated attention result.
constexpr float kSublayerAbsoluteTolerance = 4.0e-4F;
constexpr float kSublayerRelativeTolerance = 1.0e-4F;
...
bool invalid_config_tests() {
  using cuda_transformer::AttentionSublayerWorkspace;
  using cuda_transformer::NormalizedAttentionCoreConfig;
  using cuda_transformer::attention_sublayer_workspace_create;
  ...
  for (const NormalizedAttentionCoreConfig config :
    ...
    if (attention_sublayer_workspace_create(&workspace, config) !=
        cudaErrorInvalidValue) {
      std::fprintf(stderr, "invalid attention-sublayer configuration accepted\n");
      return false;
    }
  }
  return true;
...
bool run(cublasHandle_t handle, Shape shape) {
  ...
  std::vector<float> normalized(activation_count), q_token(activation_count),
      ...
      projected(activation_count), output_cpu(activation_count),
      output_gpu(activation_count), received(activation_count);
  ...
  gemm_cpu(attention_token.data(), wo.data(), projected.data(), shape.sequence,
           shape.hidden, shape.hidden);
  residual_add_cpu(input.data(), projected.data(), output_cpu.data(),
                   activation_count);

  float *d_input = nullptr, *d_norm_weight = nullptr, *d_wq = nullptr,
        *d_wk = nullptr, *d_wv = nullptr, *d_wo = nullptr, *d_output = nullptr;
  AttentionSublayerWorkspace workspace;
  auto cleanup = [&](bool ok) {
    for (float* pointer : {d_input, d_norm_weight, d_wq, d_wk, d_wv, d_wo,
                           d_output}) {
      if (pointer != nullptr && !CTR_CUDA_CHECK(cudaFree(pointer)))
        ok = false;
    }
    if (!CTR_CUDA_CHECK(attention_sublayer_workspace_destroy(&workspace)))
      ok = false;
    return ok;
  };

  if (!CTR_CUDA_CHECK(attention_sublayer_workspace_create(&workspace, config)) ||
      ...
      !CTR_CUDA_CHECK(cudaMalloc(&d_wo, weight_count * sizeof(float))) ||
      ...
      !CTR_CUDA_CHECK(cudaMemcpy(d_wv, wv.data(), weight_count * sizeof(float),
                                 cudaMemcpyHostToDevice)) ||
      !CTR_CUDA_CHECK(cudaMemcpy(d_wo, wo.data(), weight_count * sizeof(float),
                                 cudaMemcpyHostToDevice)))
    return cleanup(false);

  const AttentionSublayerWeights weights{d_norm_weight, d_wq, d_wk, d_wv, d_wo};
  bool ok = CTR_CUBLAS_CHECK(attention_sublayer_cuda(handle, d_input, weights,
                                                      &workspace, d_output)) &&
            CTR_CUDA_CHECK(cudaDeviceSynchronize());
  const std::array<std::pair<const char*, float*>, 3> device_values{{
      {"reverse repack", workspace.attention_token_major},
      {"Wo projection", workspace.projected},
      {"attention sublayer output", d_output},
  }};
  const std::array<const std::vector<float>*, 3> cpu_values{{
      &attention_token, &projected, &output_cpu,
  }};
  for (std::size_t i = 0; i < device_values.size() && ok; ++i) {
    const float absolute = i == 0 ? kAttentionAbsoluteTolerance
                                  : kSublayerAbsoluteTolerance;
    const float relative = i == 0 ? kAttentionRelativeTolerance
                                  : kSublayerRelativeTolerance;
...
int main() {
  ...
  if (!CTR_CUBLAS_CHECK(cublasDestroy(handle)))
    ok = false;
  if (ok)
    std::puts("Attention sublayer tests passed.");
