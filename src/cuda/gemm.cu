...
__global__ void gemm_tiled_kernel(const float *a, const float *b, float *c,
  ...
  const std::size_t row =
      static_cast<std::size_t>(blockIdx.y) * kGemmTileSize + local_row;
  const std::size_t column =
...
dim3 gemm_block() { return dim3(kGemmTileSize, kGemmTileSize); }

dim3 gemm_grid(std::size_t m, std::size_t n) {
  return dim3(
      static_cast<unsigned int>((n + kGemmTileSize - 1) / kGemmTileSize),
      static_cast<unsigned int>((m + kGemmTileSize - 1) / kGemmTileSize));
}

} // namespace

void gemm_cpu(const float *a, const float *b, float *c, std::size_t m,
...
cudaError_t gemm_naive_cuda(const float *a, const float *b, float *c,
                            std::size_t m, std::size_t k, std::size_t n,
                            cudaStream_t stream) {
  if (m == 0 || k == 0 || n == 0) {
  ...
  gemm_naive_kernel<<<gemm_grid(m, n), gemm_block(), 0, stream>>>(a, b, c, m, k,
                                                                  n);
  return cudaGetLastError();
}

cudaError_t gemm_tiled_cuda(const float *a, const float *b, float *c,
                            std::size_t m, std::size_t k, std::size_t n,
                            cudaStream_t stream) {
  if (m == 0 || k == 0 || n == 0) {
    return cudaErrorInvalidValue;
  ...
  gemm_tiled_kernel<<<gemm_grid(m, n), gemm_block(), 0, stream>>>(a, b, c, m, k,
                                                                  n);
  return cudaGetLastError();
}

cublasStatus_t gemm_cublas_row_major(cublasHandle_t handle, const float *a,
  ...
  return cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, static_cast<int>(n),
                     static_cast<int>(m), static_cast<int>(k), &kAlpha, b,
                     static_cast<int>(n), a, static_cast<int>(k), &kBeta, c,
                     static_cast<int>(n));
}

cublasStatus_t linear_cublas_row_major(cublasHandle_t handle,
                                       const float *input, const float *weight,
                                       float *output, std::size_t rows,
                                       std::size_t in_dim, std::size_t out_dim,
                                       cudaStream_t stream) {
  return gemm_cublas_row_major(handle, input, weight, output, rows, in_dim,
                               out_dim, stream);
}
